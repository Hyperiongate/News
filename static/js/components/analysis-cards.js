...
// Main component that creates and manages all analysis cards
...
class AnalysisCards {
    ...
    createCard(type, data, isPro) {
        ...
        switch(type) {
            ...
            case 'facts':
                ...
                break;
            case 'author':
                // Use the AuthorCard component if available
                if (window.AuthorCard) {
                    const authorCard = new window.AuthorCard();
                    card = authorCard.render(data);
                } else {
            ...
            <div class="bias-content">
                ...
                <div class="language-analysis">
                    ...
                    <div class="language-metrics">
                        ${data.language_analysis.loaded_language_count ? `
                            <div class="metric">
                                <span class="metric-icon">💭</span>
                                <span class="metric-label">Loaded Language:</span>
                                <span class="metric-value">${data.language_analysis.loaded_language_count} instances</span>
                            </div>
                        ` : ''}
                        ${data.language_analysis.emotional_tone ? `
                            <div class="metric">
                                <span class="metric-icon">😊</span>
                                <span class="metric-label">Emotional Tone:</span>
                                <span class="metric-value">${data.language_analysis.emotional_tone}</span>
                            </div>
                        ` : ''}
                    </div>
                </div>
    ...
    createAuthorCard(data) {
        // Fallback if AuthorCard component isn't loaded
                ...
                <div class="author-info">
                    ...
                    ${author.verification_status ? `
                        <div class="verification-badges">
                            ...
                            ${author.verification_status.journalist_verified ? '<span class="badge journalist">🎖️ Journalist</span>' : ''}
                        </div>
                    ` : ''}
                </div>
            </div>
    ...
    createMetricsCard(data) {
        ...
        card.innerHTML = `
            ...
            <div class="metrics-content">
                ...
                ${transparency.transparency_score !== undefined ? `
                    <div class="metrics-section">
                        ...
                        ${transparency.factors ? `
                            <ul class="transparency-factors">
                                ${Object.entries(transparency.factors).map(([key, value]) => `
                                    <li class="${value ? 'positive' : 'negative'}">
                                        ${value ? '✓' : '✗'} ${this.formatFactorName(key)}
                                    </li>
                                `).join('')}
                            </ul>
                        ` : ''}
                ...
                ${connection.coherence_score !== undefined ? `
                    <div class="metrics-section">
                        <h4>Article Coherence</h4>
                        <div class="coherence-score">
                            <span class="score-label">Logical Flow:</span>
                            <span class="score-value">${connection.coherence_score}%</span>
                        </div>
                    </div>
                ` : ''}
            </div>
