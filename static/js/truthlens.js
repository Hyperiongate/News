...
// ============================================================================
// SECTION 4: TruthLensApp Class (from app.js)
// ============================================================================
...
function renderServiceContent(service, data) {
    ...
    // Enhanced render with analysis structure
    let content = `
        <div class="service-analysis-structure">
            <div class="analysis-section">
                ...
                <div class="analysis-section-content">
                    ${getServiceAnalysisDescription(service.id)}
                </div>
            </div>
            ...
            <div class="analysis-section">
                ...
                <div class="analysis-section-content">
                    ${renderServiceFindings(service.id, data)}
                </div>
            </div>
            ...
            <div class="analysis-section">
                ...
                <div class="analysis-section-content">
                    ${getServiceInterpretation(service.id, data)}
                </div>
            </div>
        </div>
    `;
    
    // Add detailed service-specific content
    content += renderDetailedServiceContent(service.id, data);
    
    return content;
}

// Get service analysis description
function getServiceAnalysisDescription(serviceId) {
    const descriptions = {
        'source_credibility': "We're examining the publication's history, editorial standards, fact-checking processes, and track record for accuracy. This includes checking domain age, SSL certificates, and cross-referencing with media bias databases.",
        'author_analyzer': "We're investigating the author's professional background, journalism credentials, expertise in the topic area, and previous work. This includes searching for the author's profile, verifying their identity, and analyzing their publication history.",
        'bias_detector': "We're analyzing language patterns, word choices, framing techniques, and narrative structure to identify potential biases. This includes political, ideological, commercial, and confirmation biases.",
        'fact_checker': "We're extracting factual claims from the article and cross-referencing them with trusted fact-checking databases, official sources, and verified information to determine accuracy.",
        'transparency_analyzer': "We're evaluating how transparent the article is about its sources, funding, conflicts of interest, and methodology. This includes checking for proper citations and disclosure statements.",
        'manipulation_detector': "We're identifying emotional manipulation tactics, propaganda techniques, logical fallacies, and persuasion methods that might be used to influence readers unfairly.",
        'content_analyzer': "We're assessing the writing quality, structure, readability, and overall coherence of the article. This includes analyzing statistical claims, evidence quality, and logical flow.",
        'plagiarism_detector': "We're checking if the content appears elsewhere on the internet, looking for copied passages, and verifying proper attribution of sources."
    };
    
    return descriptions[serviceId] || "Analyzing this aspect of the article...";
}

// Render service findings
function renderServiceFindings(serviceId, data) {
    switch (serviceId) {
        case 'source_credibility':
            const sourceScore = extractScore(data, ['credibility_score', 'score']);
            const sourceName = data.source_name || data.name || "the source";
            return `Found that ${sourceName} has a credibility score of ${sourceScore}/100. ${data.findings ? `Key findings: ${data.findings.map(f => f.text).join(', ')}` : ''}`;
            
        case 'author_analyzer':
            const authorScore = extractScore(data, ['credibility_score', 'score']);
            const authorName = data.author_name || data.name || "the author";
            const verified = data.verification_status?.verified ? "verified" : "unverified";
            return `${authorName} has a credibility score of ${authorScore}/100 and is ${verified}. ${data.author_info?.position ? `Currently works as ${data.author_info.position}.` : ''}`;
            
        case 'bias_detector':
            const biasScore = extractScore(data, ['overall_bias_score', 'bias_score'], 0);
            const dimensions = Object.keys(data.dimensions || {}).length;
            return `Detected an overall bias score of ${biasScore}/100 across ${dimensions} dimensions. The article shows ${data.bias_level || getBiasLevel(biasScore)} levels of bias.`;
            
        case 'fact_checker':
            const normalized = DataStructureMapper.normalizeFactCheckerData(data);
            return `Analyzed ${normalized.claims_checked} factual claims. ${normalized.verified_count} were verified as accurate, resulting in ${calculateFactAccuracy(data)}% accuracy.`;
...
// Get service interpretation
function getServiceInterpretation(serviceId, data) {
    switch (serviceId) {
        case 'source_credibility':
            const sourceScore = extractScore(data, ['credibility_score', 'score']);
            if (sourceScore >= 70) {
                return "This is a reliable source with a strong track record. You can generally trust information from this publication, though always verify important claims.";
            } else if (sourceScore >= 40) {
                return "This source has mixed credibility. Be cautious and cross-reference important information with other sources.";
            } else {
                return "This source has significant credibility issues. Information should be verified through more reliable sources before accepting as fact.";
            }
            
        case 'author_analyzer':
            const authorScore = extractScore(data, ['credibility_score', 'score']);
            if (authorScore >= 70) {
                return "The author is a credible journalist with verifiable expertise. Their work typically meets professional standards.";
            } else if (authorScore >= 40) {
                return "The author has limited verifiable credentials. Consider their expertise when evaluating claims in specialized areas.";
            } else {
                return "Unable to verify the author's credentials. This doesn't necessarily mean the information is wrong, but extra caution is advised.";
            }
            
        case 'bias_detector':
            const biasScore = extractScore(data, ['overall_bias_score', 'bias_score'], 0);
            if (biasScore <= 20) {
                return "The article maintains good objectivity with minimal bias. The reporting appears balanced and fair.";
            } else if (biasScore <= 50) {
                return "Some bias is present but within acceptable journalistic bounds. Be aware of the perspective when reading.";
            } else {
                return "Significant bias detected. The article strongly favors one perspective. Seek out alternative viewpoints for balance.";
            }
            
        case 'fact_checker':
            const accuracy = calculateFactAccuracy(data);
            if (accuracy >= 80) {
                return "The factual claims are well-supported and accurate. This article can be considered reliable for its factual content.";
            } else if (accuracy >= 60) {
                return "Most facts check out, but some claims couldn't be verified. Exercise caution with specific details.";
            } else {
                return "Multiple factual issues found. Verify information independently before using or sharing.";
            }
            
        default:
            return "Review the detailed analysis below for insights.";
    }
}
...
console.log('%cPowered by 8 Specialized AI Services', 'font-size: 12px; color: #10b981;');
