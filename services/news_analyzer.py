...
class NewsAnalyzer:
    ...
    def analyze(self, content, content_type='url', is_pro=True):
        ...
        try:
            ...
            # Create enhanced bias analysis with proper domain handling
            article_domain = article_data.get('domain')
            bias_analysis = self._create_enhanced_bias_analysis(
                article_data.get('text', ''), 
                bias_score, 
                {**raw_analysis, 'domain': article_domain}
            )
    ...
    def _extract_loaded_phrases(self, text):
        """Extract loaded/biased phrases - DEPRECATED: Use _extract_enhanced_loaded_phrases"""
        return self._extract_enhanced_loaded_phrases(text)[:5]  # Limit for compatibility
    ...
    def get_basic_analysis(self, article_data):
        """Basic analysis without AI - enhanced with multi-dimensional bias detection"""
        text = article_data.get('text', '')
        
        # Multi-dimensional bias detection
        bias_dimensions = {
            'political': self._detect_political_bias(text),
            'corporate': self._detect_corporate_bias(text),
            'sensational': self._detect_sensational_bias(text),
            'nationalistic': self._detect_nationalistic_bias(text),
            'establishment': self._detect_establishment_bias(text)
        }
        
        # Use political bias as primary for backward compatibility
        bias_score = bias_dimensions['political']
        ...
        # Detect framing
        framing_analysis = self._analyze_framing_bias(text)
        framing_detected = [frame for frame, data in framing_analysis['framing_patterns'].items() 
                           if data['detected']]
        ...
        return {
            ...
            'trust_score': trust_score,
            'bias_dimensions': bias_dimensions,
            'bias_patterns': [p['type'] for p in bias_patterns],
    ...
    def _detect_bias(self, text):
        """Detect political bias in text - DEPRECATED: Use _detect_political_bias instead"""
        return self._detect_political_bias(text)
