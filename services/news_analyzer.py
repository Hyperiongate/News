...
SERVICE: News analyzer - Main analysis logic (SIMPLIFIED with BiasDetector)
"""
...
from .bias_detector import BiasDetector  # NEW IMPORT
...
class NewsAnalyzer:
    ...
    def __init__(self):
        ...
        self.bias_detector = BiasDetector()  # NEW: Initialize bias detector
    ...
    def analyze(self, content, content_type='url', is_pro=True):
        ...
        try:
            ...
            # Use the new BiasDetector for comprehensive bias analysis
            bias_analysis = self.bias_detector.analyze_comprehensive_bias(
            ...
            bias_analysis['ai_summary'] = raw_analysis.get('summary', '')
    ...
    # Delegate bias-related methods to BiasDetector for backward compatibility
    def _detect_bias(self, text):
        """Detect political bias in text"""
        return self.bias_detector.detect_political_bias(text)
    
    def _detect_manipulation(self, text):
        """Detect manipulation tactics"""
        return self.bias_detector.detect_manipulation(text)
    
    def _calculate_opinion_percentage(self, text):
        """Calculate percentage of opinion vs facts"""
        return self.bias_detector.calculate_opinion_percentage(text)
    
    def _calculate_emotional_score(self, text):
        """Calculate emotional language score"""
        return self.bias_detector.calculate_emotional_score(text)
    
    def _extract_loaded_phrases(self, text):
        """Extract loaded/biased phrases"""
        phrases = self.bias_detector.extract_loaded_phrases(text)
        return phrases[:5]  # Limit for backward compatibility
    ...
    def get_ai_analysis(self, article_data, openai_client):
        ...
        try:
            ...
            prompt = self._create_enhanced_analysis_prompt(article_data)
            ...
            response = openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert fact-checker and media analyst. Analyze articles for bias, credibility, and factual accuracy. When fact-checking claims, use your knowledge to verify them and provide clear explanations. Also analyze multiple dimensions of bias including political, corporate, sensational, and other forms."
                    },
                    {
                        "role": "user",
                        "content": prompt
            ...
            return self._parse_enhanced_ai_response(analysis_text)
            
        except Exception as e:
            logger.error(f"OpenAI API error: {str(e)}")
            return self.get_basic_analysis(article_data)
    
    def _create_enhanced_analysis_prompt(self, article_data):
        """Create enhanced analysis prompt for AI with multi-dimensional bias detection"""
        return f"""
        Analyze this news article for bias, credibility, and factual accuracy. Look for multiple types of bias.
        ...
        {{
            ...
            "bias_dimensions": {{
                "political": -1.0 to 1.0,
                "corporate": -1.0 to 1.0 (negative = anti-corporate, positive = pro-corporate),
                "sensational": 0.0 to 1.0,
                "nationalistic": -1.0 to 1.0,
                "establishment": -1.0 to 1.0
            }},
            "bias_patterns": ["cherry_picking", "strawman", "false_balance", "loaded_question", "anecdotal_evidence"],
            "framing_detected": ["victim", "hero", "threat", "progress"]
        }}
        ...
        1. Analyze all five bias dimensions, not just political
        2. Look for specific bias patterns like cherry-picking data or strawman arguments
        3. Identify how the article frames issues (victim, hero, threat, progress narratives)
        4. Consider the selection and diversity of sources quoted
        """
    
    def _parse_enhanced_ai_response(self, response_text):
        ...
        try:
            ...
            if json_match:
                ...
                # Ensure bias_dimensions exist with defaults
                if 'bias_dimensions' not in parsed:
                    parsed['bias_dimensions'] = {
                        'political': parsed.get('bias_score', 0),
                        'corporate': 0,
                        'sensational': 0,
                        'nationalistic': 0,
                        'establishment': 0
                ...
                # Ensure bias_patterns exist
                if 'bias_patterns' not in parsed:
                    parsed['bias_patterns'] = []
                
                # Ensure framing_detected exists
                if 'framing_detected' not in parsed:
                    parsed['framing_detected'] = []
    ...
    def get_basic_analysis(self, article_data):
        """Basic analysis without AI - uses BiasDetector for enhanced analysis"""
        text = article_data.get('text', '')
        
        # Use BiasDetector to get basic bias score
        bias_score = self.bias_detector.detect_political_bias(text)
        ...
        # Manipulation tactics using BiasDetector
        manipulation_tactics = self.bias_detector.detect_manipulation(text)
