...
PURPOSE: Complete universal author analyzer with all features
...
class AuthorAnalyzer:
    ...
    def analyze_single_author(self, author_name, domain=None):
        ...
        try:
            ...
            if cached:
                ...
                cached_data = {
                    'name': cached.name,
                    'found': True,
                    'bio': cached.bio or f"{cached.name} is a journalist.",
        ...
        try:
            ...
            if result['found'] and result['credibility_score'] > 30:
        ...
        logger.info(f"Analysis complete for {clean_name}: Found={result['found']}, Score={result['credibility_score']}, Sources={result['sources_checked']}")
    ...
    def _enhanced_web_search(self, author_name, domain=None):
        ...
        if all_snippets:
            ...
            if bio_candidates:
                result['bio'] = max(bio_candidates, key=len)[:500]  # Limit length
            elif all_snippets:
                result['bio'] = all_snippets[0][:500]
